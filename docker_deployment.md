...
```
You can use a more useful variant such as `docker ps -a` instead of `docker ps` if you do not see any output.

Note that the CONTAINER ID is shortened compared to what was generated by `docker run` command. 
...
An application normally has a number of configuration that are sourced from environment variables. For example, the Flask secret key, database URL and email server options are all imported from environment variables.
